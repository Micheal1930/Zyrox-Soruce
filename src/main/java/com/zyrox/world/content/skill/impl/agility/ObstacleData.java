package com.zyrox.world.content.skill.impl.agility;
			...
			TaskManager.submit(new Task(1, player, false) {
				...
				public void stop() {
					...
					if(Agility.passedAllObstacles(player)) {
						...
						player.getInventory().add(2996, 2, "Agility ticket");
			...
			TaskManager.submit(new Task(1, player, true) {
				...
				public void stop() {
					...
					if(!fail) {
						...
						Agility.addExperience(player, 13.5);
						player.getUpdateFlag().flag(Flag.APPEARANCE);
		...
		public void cross(final Player player) {
			...
			TaskManager.submit(new Task(2, player, false) {
				...
				public void execute() {
					...
					Agility.addExperience(player, 8);
				...
				public void execute() {
					...
					if(player.getPosition().getX() == 2543 && player.getPosition().getY() == 3553) {
						if(Agility.passedAllObstacles(player)) {
							...
							player.getInventory().add(2996, 4, "Agility ticket");
							Agility.addExperience(player, 46.5);
							Agility.resetProgress(player);
						} else {
				...
				public void execute() {
					...
					if(tick == 4 || tick == 7 || tick == 10 || tick == 13 || tick == 16) {
						...
						if(player.getLocation() == Location.WILDERNESS) {
							...
							player.moveTo(new Position(2768, 10002, 0));
						}
						stop();
					}
				}
		...
		public void cross(final Player player) {
			...
			TaskManager.submit(new Task(2, player, false) {
				...
				public void stop() {
					...
					if(Agility.passedAllObstacles(player)) {
						DialogueManager.start(player, 57);
						player.getInventory().add(2996, 6, "Agility ticket");
						Agility.addExperience(player, 489.9);
					} else {
						DialogueManager.start(player, 56);
					}

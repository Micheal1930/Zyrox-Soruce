...
import com.zyrox.engine.task.impl.NPCRespawnTask;
...
public class PuroPuro {
	...
	public static void spawn() {
		...
		for(int i = 0; i < implings.length; i++) {
			NPC n = NPC.of(implings[i][0], new Position(implings[i][1], implings[i][2]));
	...
	public static void catchImpling(Player player, final NPC imp) {
		...
		if(sucess) {
			if(imp.isRegistered()) {
				...
				TaskManager.submit(new NPCRespawnTask(imp, imp.getDefinition().getRespawnTime(), player));
				player.getInventory().delete(11260, 1).add(implingData.impJar, 1, "Hunter catch impling");
				player.getPacketSender().sendMessage("You successfully catch the impling.");
				player.getSkillManager().addExperience(Skill.HUNTER, implingData.XPReward * Skill.HUNTER.getModifier());
	...
	public static void lootJar(final Player player, Item jar, JarData jarData) {
		...
		player.getInventory().add(11260, 1, "Hunter loot jar");
		...
		case 0:
			...
			if(reward != null)
				player.getInventory().add(reward, "Impling jar loot");
			break;
		case 1:

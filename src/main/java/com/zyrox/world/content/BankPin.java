...
public class BankPin {
	...
	public static final int INTERFACE_ID = 7424;
	
	private static final Set<String> FORBIDDEN_PINS = new HashSet<String>(Arrays.asList("0000", "1111", "2222", "3333", "4444", "5555", "6666", "7777", "8888", "9999", "1234", "4321"));
	...
	public static void init(Player player, boolean openBankAfter) {
		if (player.getBankPinAttributes().getInvalidAttempts() == 3) {
			if(System.currentTimeMillis() - player.getBankPinAttributes().getLastAttempt() < 400000) {
				player.getPacketSender().sendMessage("<img=483> You must wait "+(int)((400 - (System.currentTimeMillis() - player.getBankPinAttributes().getLastAttempt()) * 0.001))+" seconds before attempting to enter your bank-pin again.");
				return;
			} else
	...
	public static void clickedButton(Player player, int button) {
		...
		if(player.getBankPinAttributes().getEnteredBankPin()[0] == -1) {
			...
			if(!player.getBankPinAttributes().hasBankPin()) {
				if (FORBIDDEN_PINS.contains(player.getBankPinAttributes().getEnteredBankPin()[0] + "" + player.getBankPinAttributes().getEnteredBankPin()[1] + "" + player.getBankPinAttributes().getEnteredBankPin()[2] + "" + player.getBankPinAttributes().getEnteredBankPin()[3])) {
					player.getPacketSender().sendInterfaceRemoval();
					player.getPacketSender().sendMessage("Please choose a more complex bank pin!");
					init(player, false);
					return;
				}
				player.getBankPinAttributes().setHasBankPin(true).setHasEnteredBankPin(true).setBankPin(player.getBankPinAttributes().getEnteredBankPin()).setLastEnteredBankPin(System.currentTimeMillis());;
				player.getPacketSender().sendMessage("<img=483> You've created a bank-pin. Your digit is "+player.getBankPinAttributes().getEnteredBankPin()[0]+"-"+player.getBankPinAttributes().getEnteredBankPin()[1]+"-"+player.getBankPinAttributes().getEnteredBankPin()[2]+"-"+player.getBankPinAttributes().getEnteredBankPin()[3]+". Please write it down.");
				player.getPacketSender().sendInterfaceRemoval();
				return;
			}
			for(int i = 0; i < player.getBankPinAttributes().getEnteredBankPin().length; i++) {
				if(player.getBankPinAttributes().getEnteredBankPin()[i] != player.getBankPinAttributes().getBankPin()[i]) {
					...
					player.getPacketSender().sendMessage("Invalid bank-pin entered.");
					return;
				}
			}
			player.getBankPinAttributes().setInvalidAttempts(0).setHasEnteredBankPin(true).setLastEnteredBankPin(System.currentTimeMillis());
			player.getPacketSender().sendInterfaceRemoval();
			if (player.openBank()) {
	...
	public static boolean isBankPinButton(int buttonId) {
		for (int i = 0; i < actionButtons.length; i++) {
			if (actionButtons[i] == buttonId) {
				return true;
			}
		}
	...
	public static class BankPinAttributes {
		...
		private long lastEnteredBankPin;
		...
		public long getLastEnteredBankPin() {
			return lastEnteredBankPin;
		}
		
		public void setLastEnteredBankPin(long lastEnteredBankPin) {
			this.lastEnteredBankPin = lastEnteredBankPin;
		}

		public int[] getBankPin() {
			return bankPin;
		}
		public int[] getBankPin2() {
			return bankPin.clone();
		}

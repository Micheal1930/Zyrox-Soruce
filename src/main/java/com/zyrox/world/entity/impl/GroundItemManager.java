package com.zyrox.world.entity.impl;

import java.util.concurrent.CopyOnWriteArrayList;

import com.zyrox.engine.task.impl.GroundItemsTask;
...
public class GroundItemManager {
	...
	public static void spawnGroundItem(Player p, GroundItem g) {
		...
		if (ItemDefinition.forId(item.getId()).isStackable()) {
			...
			if (it != null) {
				int currentItemAmount = it.getItem().getAmount();
				long totalAmount = (long) g.getItem().getAmount() + currentItemAmount;
	 ...
	 * @param position
	 */
	public static boolean pickupGroundItem(Player p, Item item, Position position) {
		...
		if(gt == null || gt.hasBeenPickedUp() || !groundItems.contains(gt)) //last one isn't needed, but hey, just trying to be safe
			...
			gt.setPickedUp(true);
			remove(gt, true);
			p.getInventory().add(item, "pickup ground item " + item.getId()+ "/" + item.getAmount());
	...
	public static GroundItem getGroundItem(Player p, Item item, Position position) {
		for(GroundItem l : groundItems) {
			if(l == null || l.getPosition().getZ() != position.getZ())
				continue;
			if(l.getPosition().sameAs(position) && l.getItem().getId() == item.getId()) {
				if(l.isGlobal())
					return l;
				else if(p != null) {
	...
	public static void clearArea(Position pos, String owner) {
		for(GroundItem l : groundItems) {
			if(l == null || l.getPosition().getZ() != pos.getZ())
				continue;
			if(l.getPosition().sameAs(pos) && l.getOwner().equals(owner)) 
				remove(l, true);
		}
	}

...
public final class NpcAggression {
	...
	public static void target(Player player) {
		...
		if(player.isPlayerLocked() || player.requiresUnlocking() || player.isSpectatorMode() || player.getTutorialStage() != TutorialStages.COMPLETED)
			return;
		...
		for (NPC npc : player.getLocalNpcs()) {
			...
			if(npc != null  && npc instanceof AncestralGlyph) {
				continue;
			}
			
			NPCFacing.updateFacing(player, npc);
			if(npc == null || npc instanceof MaxHitDummy || npc.getConstitution() <= 0 || !(dung && npc.getId() != 11226) && npc.getDefinition() != null && !npc.getDefinition().isAggressive()) {
				continue;
			}
			...
			/** GWD **/
			/*boolean gwdMob =
					Nex.nexMob(npc.getId()) || npc.getId() == 6260 || npc.getId() == 6261 || npc.getId() == 6263 || npc.getId() == 6265 || npc.getId() == 6222 || npc.getId() == 6223 || npc.getId() == 6225 || npc.getId() == 6227 || npc.getId() == 6203 || npc.getId() == 6208 || npc.getId() == 6204 || npc.getId() == 6206 || npc.getId() == 6247 || npc.getId() == 6248 || npc.getId() == 6250 || npc.getId() == 6252;
			if(gwdMob) {
			...
			if (Locations.goodDistance(npc.getPosition(), player.getPosition(), npc.getAggressionDistance()) /*|| gwdMob*/) {
				...
				if (player.getTolerance().elapsed() > (NPC_TOLERANCE_SECONDS * 1000) /*&& player.getLocation() != Location.GODWARS_DUNGEON */&&
				...
				if(Location.ignoreFollowDistance(npc) /*|| gwdMob*/ || npc.getDefaultPosition().getDistance(player.getPosition()) < 7 + npc
				.getMovementCoordinator().getCoordinator().getRadius() || dung) {

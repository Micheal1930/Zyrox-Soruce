...
import com.varrock.model.input.impl.EnterAmountToInfuse;
			...
			public void execute() {
				...
				while(x > 0) {
					if(!hasRequirements(player, pouch))
						...
						player.getSkillManager().addExperience(Skill.SUMMONING, pouch.getExp() * Skill.SUMMONING.getModifier());
						player.getInventory().add(pouch.getPouchId(), 1, "Infuse pouches");

...
public class WeaponPoison {
		...
		if (weapon != null) {
			for (int element[] : weapon.getNewItemId())
				if (itemUse == element[0] && player.getInventory().contains(itemUse)) {
					...
					player.getInventory().add(229, 1, "Weapon poisoning");
					player.getInventory().add(element[1], 1, "Weapon poisoning");
				}
		}
	}
	 ...
	 */
	public static void handleWeaponPoison(Player p, GameCharacter target) {

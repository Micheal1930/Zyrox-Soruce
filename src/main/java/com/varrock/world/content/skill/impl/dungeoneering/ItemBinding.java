...
public class ItemBinding {
	...
	public static void bindItem(Player p, int item) {
		if(Dungeoneering.doingDungeoneering(p)) {
			...
			if(amountBound >= 4) {
				...
				return;
			} else if(amountBound == 3 && p.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING) < 110) {
				p.getPacketSender().sendMessage("You need a Dungeoneering level of at least 110 to have 4 bound items.");
				return;
			...
			if(bind != -1 && index != -1) {
				...
				p.getInventory().delete(item, 1).add(bind, 1, "Bind item");
	...
	public static void onDungeonEntrance(Player p) {
		for(int i = 0; i < p.getMinigameAttributes().getDungeoneeringAttributes().getBoundItems().length; i++) {
			if(p.getMinigameAttributes().getDungeoneeringAttributes().getBoundItems()[i] != 0) {
				p.getInventory().add(p.getMinigameAttributes().getDungeoneeringAttributes().getBoundItems()[i], 1, "Item binding");
			}
		}
	}
}

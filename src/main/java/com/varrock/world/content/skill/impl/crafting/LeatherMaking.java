...
import com.varrock.model.input.impl.EnterAmountOfLeatherToCraft;
		...
		if (l.getLeather() == player.getSelectedSkillingItem()) {
			...
			player.setCurrentTask(new Task(2, player, true) {
				...
				public void execute() {
					...
					player.getInventory().delete(l.getLeather(), l.getHideAmount()).add(l.getProduct(), 1, "Leather making");
					player.getSkillManager().addExperience(Skill.CRAFTING, l.getXP() * Skill.CRAFTING.getModifier());

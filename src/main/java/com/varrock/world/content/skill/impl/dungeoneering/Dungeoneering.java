package com.varrock.world.content.skill.impl.dungeoneering;
...
public class Dungeoneering {
	...
	public static boolean DUNGEONEERING_ENABLED = true;

	public static void start(final Player p) {
		if (!DUNGEONEERING_ENABLED) {
			p.getPacketSender().sendMessage("Dungeoneering has been disabled until further notice.");
			return;
		}
		...
		for (Player member : party.getPlayers()) {
			if (member != null) {
				...
				if (member.getGameMode() != GameMode.ULTIMATE_IRONMAN) {
		...
		for (Player member : party.getPlayers()) {
			...
			if (member.getGameMode() == GameMode.ULTIMATE_IRONMAN) {
				Bank.depositItems(member, member.getEquipment(), true);
				Bank.depositItems(member, member.getInventory(), true);
			}
			member.getPacketSender().sendInterfaceRemoval();
			...
			member.getInventory().add(18201, amt, "Dungeoneering start");
		...
		p.getInventory().add(new Item(17489), "Dungeoneering begin");
	}
	...
	public static void leave(Player p, boolean resetTab, boolean leaveParty) {
		if (p.getMinigameAttributes().getDungeoneeringAttributes().getParty() != null) {
			...
			p.getPacketSender().sendTabInterface(GameSettings.QUESTS_TAB, 23508);
	...
	public static void setupFloor(DungeoneeringParty party, int height) {
		 ...
		 * Spawning npcs
		 */
		NPC smuggler = NPC.of(11226, new Position(party.getDungeoneeringFloor().getSmugglerPosition().getX(),
		...
		for (NPC n : party.getDungeoneeringFloor().getNpcs()[party.getComplexity() - 1]) {
			NPC npc = NPC.of(n.getId(), n.getPosition().copy().setZ(height));
			World.register(npc);

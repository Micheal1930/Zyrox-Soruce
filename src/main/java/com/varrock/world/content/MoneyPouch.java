...
public class MoneyPouch {
	...
	public static boolean withdrawMoney(Player plr, long amount) {
		if(plr.getMinigame() != null || plr.isInsideRaids() || Dungeoneering.doingDungeoneering(plr)) {
			plr.getPacketSender().sendMessage("You cannot withdraw money from your pouch inside of minigames.");
			return false;
		}
		if (plr.requiresUnlocking()) {
			BankPin.init(plr, false);
			return false;
		}
		...
		boolean allowWithdraw = plr.getBank().isOpen() || plr.getTrading().inTrade() || plr.getDueling().inDuelScreen;
		...
		if (!plr.getInventory().contains(995) && plr.getInventory().isFull()) {
		...
		if ((plr.getInventory().getAmount(995) + amount) <= Integer.MAX_VALUE) {
			plr.setMoneyInPouch(plr.getMoneyInPouch() - amount);
			plr.getInventory().add(995, (int) amount, "Money pouch withdraw");
			plr.getPacketSender().sendString(8135, "" + plr.getMoneyInPouch());
			...
			plr.getInventory().add(995, canWithdraw, "Money pouch withdraw");
			plr.getPacketSender().sendString(8135, "" + plr.getMoneyInPouch());
	...
	public static void toBank(Player player) {
		...
		if(totalAmount > Integer.MAX_VALUE || totalAmount < 0) {
			...
			player.getBank(Bank.getTabForItem(player, 995)).add(995, canWithdraw, "Money pouch to bank");
			player.getPacketSender().sendString(8135, ""+player.getMoneyInPouch());
			player.getPacketSender().sendMessage("You could only withdraw "+canWithdraw+" coins.");
		} else {
			player.getBank(player.getCurrentBankTab()).add(995, amount, "Money pouch to bank");

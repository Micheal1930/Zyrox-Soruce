...
public class Scoreboards {
	...
	/**
	 * The amount of results to display in the scoreboard.
	 */
	private static final int AMOUNT_OF_RESULTS = 10;
	...
	public static ArrayList<ScoreboardEntry> getUpdatedList(Scoreboard scoreboard) {
		...
		for (Player p : World.getPlayers()) {
			...
			if (scoreboard == Scoreboards.TOP_PKERS) {
				...
				entries.add(new ScoreboardEntry(p.getUsername(),
						new String[] { String.valueOf(p.getPlayerKillingAttributes().getKillstreak()) }));
	...
	public static void open(Player player, Scoreboard scoreboard) {
		...
		for (int i = 0; i < 10; i++) {
			player.getPacketSender().sendString(6402 + i, "");
		}

		for (int x = 8578; x < 8700; x++)
			player.getPacketSender().sendString(x, "");

		for (int i = 0; i < AMOUNT_OF_RESULTS; stringId++, i++) {
			if (i == 10) {
				stringId = 8578;
			}

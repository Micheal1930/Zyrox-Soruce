...
public class ItemsKeptOnDeath {
	...
	public static void sendInterfaceData(Player player) {
		...
		for(Item item : Misc.concat(player.getInventory().getItems(), player.getEquipment().getItems())) {
			if(item == null || item.getId() <= 0 || item.getAmount() <= 0 || !item.tradeable(player) || toKeep.contains(item)) {
				continue;
			}
			if(toSend >= 17142)
				toSend = 17150;
			if(toSend >= 17160)
				break;
			if (item.getId() == Items.RUNE_POUCH && player.getLocation() != Location.WILDERNESS) {
				continue;
			}
	...
	public static ArrayList<Item> getItemsToKeep(Player player) {
		...
		if (keepRunePouch) {
			toKeep.add(new Item(Items.RUNE_POUCH));
		}
		return toKeep;
	}

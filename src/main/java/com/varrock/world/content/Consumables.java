...
import com.varrock.engine.task.impl.FireImmunityTask;
import com.varrock.engine.task.impl.OverloadPotionTask;
import com.varrock.engine.task.impl.PoisonImmunityTask;
import com.varrock.engine.task.impl.PrayerRenewalPotionTask;
...
public class Consumables {
	 ...
	 * @return is food
	 */
	public static boolean isFood(Player player, int item) {
		FoodType food = FoodType.types.get(item);
		return food != null;
	}

	public static int lastfood;
	...
	private static void eat(Player player, FoodType food, int slot) {
        ...
        if (player.getFoodTimer().elapsed() < 1100 && (food != FoodType.KARAMBWAN || lastfood == 3144)) {
            return;
        }
        ...
        if (food == FoodType.CAKE || food == FoodType.SECOND_CAKE_SLICE) {
            player.getInventory().add(new Item(food.item.getId() + 2, 1), "cake slices ate");
	...
	private enum FoodType {
		...
		KARAMBWAN(new Item(3144), 180), ANGLERFISH(new Item(43441), 220), DARK_CRAB(new Item(41936), 220),
		ROCKTAIL(new Item(15272), 230),
		SCYTHE_OF_VITUR_A(new Item(52325), 230),
		/*
		 ...
		 * Other food types.
		 */
		PURPLE_SWEETS(new Item(4561), 130), OKTOBERTFEST_PRETZEL(new Item(19778), 120);
	...
	public static void handlePotion(final Player player, final int itemId, final int slot) {
		...
		if (player.getPotionTimer().elapsed(1700)) {
			switch (itemId) {
			/*
			 * Attack potions
			 */
			...
			case 9745:
				...
				break;
			case 42695:
				drinkStatPotion(player, itemId, 42697, slot, 0, true);
				drinkStatPotion(player, itemId, 42697, slot, 1, true);
				drinkStatPotion(player, itemId, 42697, slot, 2, true);
				drinkStatPotion(player, itemId, 42697, slot, 4, true);
				drinkStatPotion(player, itemId, 42697, slot, 6, true);
				break;
			case 42697:
				drinkStatPotion(player, itemId, 42699, slot, 0, true);
				drinkStatPotion(player, itemId, 42699, slot, 1, true);
				drinkStatPotion(player, itemId, 42699, slot, 2, true);
				drinkStatPotion(player, itemId, 42699, slot, 4, true);
				drinkStatPotion(player, itemId, 42699, slot, 6, true);
				break;
			case 42699:
				drinkStatPotion(player, itemId, 42701, slot, 0, true);
				drinkStatPotion(player, itemId, 42701, slot, 1, true);
				drinkStatPotion(player, itemId, 42701, slot, 2, true);
				drinkStatPotion(player, itemId, 42701, slot, 4, true);
				drinkStatPotion(player, itemId, 42701, slot, 6, true);
				break;
			case 42701:
				drinkStatPotion(player, itemId, -1, slot, 0, true);
				drinkStatPotion(player, itemId, -1, slot, 1, true);
				drinkStatPotion(player, itemId, -1, slot, 2, true);
				drinkStatPotion(player, itemId, -1, slot, 4, true);
				drinkStatPotion(player, itemId, -1, slot, 6, true);
				break;
			/*
			 * Agility potions
			 */
			...
			case 5943: // Antipoison+ pot 4
				...
				player.getInventory().refreshItems();
				PoisonImmunityTask.makeImmune(player, 360);
				player.setVenomDamage(0);
				player.getPacketSender().sendConstitutionOrbVenom(false);
				player.getPacketSender()
			 ...
			 * Antipoison++ potions
			 */
			case 5952: // Antipoison++ pot 4
				player.performAnimation(new Animation(829));
				player.getInventory().getItems()[slot] = new Item(5954, 1);
				player.getInventory().refreshItems();
				PoisonImmunityTask.makeImmune(player, 720);
				player.setVenomDamage(0);
				player.getPacketSender().sendConstitutionOrbVenom(false);
				player.getPacketSender()
						.sendMessage("You're now immune to any kind of poison for another 650 seconds.");
				break;
			case 5954: // Antipoison++ pot 3
				player.performAnimation(new Animation(829));
				player.getInventory().getItems()[slot] = new Item(5956, 1);
				player.getInventory().refreshItems();
				PoisonImmunityTask.makeImmune(player, 720);
				player.setVenomDamage(0);
				player.getPacketSender().sendConstitutionOrbVenom(false);
				player.getPacketSender()
						.sendMessage("You're now immune to any kind of poison for another 650 seconds.");
				break;
			case 5956: // Antipoison++ pot 2
				player.performAnimation(new Animation(829));
				player.getInventory().getItems()[slot] = new Item(5958, 1);
				player.getInventory().refreshItems();
				PoisonImmunityTask.makeImmune(player, 720);
				player.setVenomDamage(0);
				player.getPacketSender().sendConstitutionOrbVenom(false);
			...
			case 2450:
                ...
                if (player.getSkillManager().getCurrentLevel(Skill.forId(5)) > player.getSkillManager().getMaxLevel(Skill.forId(5))) {
                    player.getSkillManager().setCurrentLevel(Skill.forId(5), player.getSkillManager().getMaxLevel(Skill.forId(5)));
                }
				break;
			case 189:
				if (player.getConstitution() < 100) {
			...
			case 6691:
				...
				player.getInventory().getItems()[slot] = new Item(nextId, 1);
                doTheBrew(player);
				break;
			/*
			 * Restore potions
	...
	private static void doTheBrew(Player player) {
		...
		Skill hitpoints = Skill.forId(3);
		if (hitpoints == null || player.getSkillManager().getCurrentLevel(hitpoints) >= (player.getSkillManager().getMaxLevel(hitpoints) * amount + 1)) {
			return;
		}
		player.getSkillManager().setCurrentLevel(Skill.forId(3),
				player.getSkillManager().getCurrentLevel(hitpoints) + bonus);
		if (player.getSkillManager().getCurrentLevel(hitpoints) > (player.getSkillManager().getMaxLevel(hitpoints) * amount + 1)) {
			player.getSkillManager().setCurrentLevel(Skill.forId(3), (int) (player.getSkillManager().getMaxLevel(hitpoints) * amount));
		}
	}

...
import com.varrock.engine.task.impl.CeilingCollapseTask;
import com.varrock.model.GameObject;
import com.varrock.model.Item;
import com.varrock.model.Position;
import com.varrock.model.RegionInstance;
	...
	public static boolean handleObject(final Player player, GameObject object) {
		...
		case 10284:
			...
			if (player.getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player.getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][1] == 0) {
				...
				resetBarrows(player);
				player.getInventory().add(randomRunes(),  25 + Misc.getRandom(70), "Barrows runes");
				if (Misc.getRandom(100) >= 60) {
					int b = randomBarrows();
					int s = randomSets();
					if(Misc.getRandom(100) == 4) {
						player.getInventory().add(s, 1, "Barrows");
					}
					player.getInventory().add(b, 1, "Barrows");
					World.sendMessage("<img=483> @blu@[ Barrows ]: <col=E9E919><shad=0> "+player.getUsername()+" has just received "+ItemDefinition.forId(b).getName()+" from the Barrows!</col>");
				}
	...
	public static void searchCoffin(final Player player, final int obj, final int coffinId, int npcId, Position spawnPos) {
		...
		if (player.getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[coffinId][1] == 0) {
			if(player.getLocation() == Location.BARROWS) {
				...
				NPC npc_ = NPC.of(npcId, spawnPos);

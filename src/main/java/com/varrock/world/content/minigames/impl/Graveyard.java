...
public class Graveyard {
	...
	private final static void spawn(Player player, int wave, int level) {
		...
		TaskManager.submit(new Task(4, player, false) {
			...
			public void execute() {
				...
				player.getPacketSender().sendMessage("@blu@Starting wave:@red@  "+wave);
				
				int totalAmount = zombieAmount +1;
			
				player.getPacketSender().sendMessage("@blu@Zombies to kill:@red@ "+totalAmount);
				player.getPA().sendMessage("----------------");
				for(int i = 0; i <= zombieAmount; i++) {
					NPC n = NPC.of(getSpawn(level), getSpawnPos(player.getPosition().getZ())).setSpawnedFor(player);
					World.register(n);
	...
	public static boolean handleDeath(Player player, NPC npc) {
		...
		if(amount > 0) {
			...
			player.getInventory().add(new Item(14667, amount));

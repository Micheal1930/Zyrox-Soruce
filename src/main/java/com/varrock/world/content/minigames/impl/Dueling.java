package com.varrock.world.content.minigames.impl;
...
import com.varrock.model.log.impl.StakeItemLog;
...
public class Dueling {
	...
	public void challengePlayer(Player playerToDuel) {
		if(!GameSettings.DUEL_ENABLED) {
			player.getPacketSender().sendMessage("Dueling is disabled at the moment.");
			return;
		}
	...
	public void declineDuel(boolean tellOther) {
		...
		for (Item item : stakedItems) {
			if (item.getAmount() < 1)
				continue;
			player.getInventory().add(item, "Duel decline");
		}
		reset();
	...
	public void stakeItem(int itemId, int amount, int slot) {
        ...
        if (playerToDuel.getGameMode() != GameMode.NORMAL) {
		...
		if (!ItemDefinition.forId(itemId).isStackable()) {
			...
			for (Item item : stakedItems) {
				if (item.getId() == itemId) {
					...
					if(amount == 0) {
						player.getPacketSender().sendMessage("You can't add anymore");
						break;
					}
				...
				for (Item item : stakedItems) {
					if (item.getId() == itemId) {
						if (!item.getDefinition().isStackable()) {
							...
							player.getInventory().add(item, "remove staked item");
							}
						break;
					}
				}
				...
				if (item.getId() == itemId) {
					if (item.getDefinition().isStackable()) {
						if (item.getAmount() > amount) {
							item.setAmount(item.getAmount() - amount);
							player.getInventory().add(itemId, amount, "Remove staked item");
						} else {
							amount = item.getAmount();
							stakedItems.remove(item);
							player.getInventory().add(item.getId(), amount, "Remove staked item");
						}
					}
					break;
				}
		...
		if(DuelRule.forButtonId(button) != null) {
			...
			case 6520:
				...
				if (playerToDuel.getDueling().duelingStatus == 4 && player.getDueling().duelingStatus == 4) {
					...
					player.getDueling().duelingAtHome = player.getPosition().isWithinBoundary( 3073, 3081, 3484, 3501);
					playerToDuel.getDueling().duelingAtHome = playerToDuel.getPosition().isWithinBoundary( 3073, 3081, 3484, 3501);
		...
		for(int i = 10; i < selectedDuelRules.length; i++) {
			...
			if(selectedDuelRules[i]) {
				...
				if(player.getEquipment().getItems()[rule.getEquipmentSlot()].getId() > 0) {
					...
					player.getInventory().add(equipItem, "duel start equip");
				}
			}
		}
	...
	public void duelVictory() {
			...
			if(playerDuel != null && playerDuel.getDueling().stakedItems.size() > 0) {
				for(Item item : playerDuel.getDueling().stakedItems) {
					if(item.getId() > 0 && item.getAmount() > 0) {
						if(refund) {
							playerDuel.getInventory().add(item, "Dueling Reward");
						} else {
							new StakeItemLog(playerDuel.getName(), player.getName(), item.getDefinition().getName(), item.getId(), item.getAmount(), Misc.getTime()).submit();
		...
		for (Item item : stakedItems) {
			if (item.getId() > 0 && item.getAmount() > 0) {
				player.getInventory().add(item, "Duel victory");
				new StakeItemLog(player.getName(), player.getName(), item.getDefinition().getName(), item.getId(), item.getAmount(), Misc.getTime()).submit();
			}
		}
		reset();
		arenaStats[0]++;
	...
	public static boolean checkDuel(Player playerToDuel, int statusReq) {
		...
		if(playerToDuel.getDueling().duelingStatus != statusReq || playerToDuel.isBanking() || playerToDuel.getConstitution() <= 0 || playerToDuel.isResting() || !goodInterfaceId)
			return false;
		return true;
	}
	...
	public boolean duelingAtHome = false;

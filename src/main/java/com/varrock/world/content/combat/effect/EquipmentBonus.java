package com.varrock.world.content.combat.effect;
...
public class EquipmentBonus {
	...
	public static boolean wearingEliteVoid(Player player, CombatType attackType) {
		int helmet = attackType == CombatType.MAGIC ? MAGE_VOID_HELM :
						attackType == CombatType.RANGED ? RANGED_VOID_HELM : MELEE_VOID_HELM;
		boolean eliteVoid = true;
		for (int elite : ELITE_VOID_ARMOUR) {
			if (!player.getEquipment().contains(elite)) {
				eliteVoid = false;
			}
		}
		if (!player.getEquipment().contains(8842)) {
			eliteVoid = false;
		}
		return eliteVoid && player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == helmet;
	}

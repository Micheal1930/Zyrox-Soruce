...
public class CombatBuilder {
	 ...
	 *            the entity controlling this builder.
	 */
	public CombatBuilder(GameCharacter entity) {
	...
	public void attack(GameCharacter target) {
		...
		if (character.equals(target) || !target.canTakeDamage() || (character.isPlayer() && character.asPlayer().isSpectatorMode())) {
			return;
		}
		...
		//If the target is a player and currently in agility cancel attack.
        if (target.isPlayer()) {
            if (target.asPlayer().isInAgility()) {
                return;
            }
       }
	...
	public Player getKiller(boolean clearMap) {
		...
		for (Entry<Player, CombatDamageCache> entry : damageMap.entrySet()) {
			...
			// Check if the key for this has logged
			// out.
	 ...
	 */
	public void addDamage(GameCharacter entity, int amount) {
	 ...
	 *            the entity that last attacked you.
	 */
	public void setLastAttacker(GameCharacter lastAttacker) {
	...
	public CombatContainer getContainer() {
		if (this.container != null)
			return container;
		return strategy == null ? null : strategy.attack(character, victim);
	}

...
public class Nex implements CombatStrategy {
	...
	public static Nex globalNex = new Nex();

	public static Nex getNex(Player player) {
		if(player == null)
			return globalNex;
		return player.nexInstance != null && player.getPosition().getZ() >= 4 ? player.nexInstance : globalNex;
	}
	
	public NPC NEX;

	public NPC FUMUS, UMBRA, CRUOR, GLACIES;
	...
	private boolean zarosStage;
	private int phase, prayerType, prayerTimer;

	public void spawn(int heightLevel) {
		...
		NEX = NPC.of(13447, new Position(2925, 5203, heightLevel));
		NEX.getMovementCoordinator().setCoordinator(new Coordinator(true, 3));

		FUMUS = NPC.of(13451, new Position(2916, 5213, heightLevel));
		UMBRA = NPC.of(13452, new Position(2934, 5213, heightLevel));
		CRUOR = NPC.of(13453, new Position(2915, 5193, heightLevel));
		GLACIES = NPC.of(13454, new Position(2935, 5193, heightLevel));
	...
	public ArrayList<NPC> getNpcList() {
		...
		list.addAll(Arrays.asList(NEX, FUMUS, UMBRA, CRUOR, GLACIES));
		return list;
	}

	public void despawn(boolean nex) {
		if (nex) {
	...
	public void death(Player player, final int id) {
		if (nexMinion(id)) {
			int index = id - 13451;
			if (index >= 0) {
			...
			if(player.getPosition().getZ() > 0) {
				player.getPacketSender().sendMessage("Re-enter the instance to spawn the Nex boss again.");
				return;
			}
	...
	public boolean customContainerAttack(GameCharacter entity, GameCharacter victim) {
		...
		NPC nexNpc = (NPC) entity;
		if(NEX == null) {
			NEX = nexNpc;
		}
		if (p.getConstitution() <= 0 || nexNpc.getConstitution() <= 0) {
			return true;
		}
		if (nexNpc.isChargingAttack() || p.getConstitution() <= 0) {
			return true;
		}
		if (phase == 0) {
			int rnd = Misc.getRandom(10);
			if (rnd <= 2) {
				nexNpc.forceChat("Let the virus flow through you!");
				cough(p);
				nexNpc.performAnimation(new Animation(6986));
				new CombatHitTask(nexNpc.getCombatBuilder(), new CombatContainer(NEX, p, 1, CombatType.MAGIC, true))
						.handleAttack();
				return true;
			}
			if (p.getPosition().distanceToPoint(nexNpc.getPosition().getX(), nexNpc.getPosition().getY()) <= 2
					&& Misc.getRandom(1) == 0) {
				nexNpc.performAnimation(new Animation(6354));
				TaskManager.submit(new Task(1, NEX, false) {
					@Override
					public void execute() {
						new CombatHitTask(nexNpc.getCombatBuilder(),
				...
				nexNpc.performAnimation(new Animation(6326));
		...
		if (phase == 1) {
			...
			if (rnd < 2 && !attacks[3]) {
				nexNpc.forceChat("Fear the shadow!");
				attacks[3] = true;
				nexNpc.setChargingAttack(true);
				for (final Player p_ : Misc.getCombinedPlayerList(p)) {
					...
					TaskManager.submit(new Task(1, NEX, false) {
						...
						public void stop() {
							...
							attacks[3] = false;
							nexNpc.setChargingAttack(false);
						}
					});
				}
			} else if (rnd >= 5 && rnd <= 7 && !attacks[4]) {
				nexNpc.forceChat("Embrace darkness!");
				attacks[4] = true;
				nexNpc.setChargingAttack(true);
					...
					TaskManager.submit(new Task(1, NEX, false) {
						...
						public void execute() {
							if (ticks == 10)
								...
								double dist = p_.getPosition().distanceToPoint(nexNpc.getPosition().getX(),
										nexNpc.getPosition().getY());
								if (dist < 3) {
				...
				nexNpc.performAnimation(new Animation(6984));
				new CombatHitTask(nexNpc.getCombatBuilder(), new CombatContainer(NEX, p, 1, CombatType.MAGIC, true))
						.handleAttack();
				return true;
			} else {
				if (p.getPosition().distanceToPoint(nexNpc.getPosition().getX(), nexNpc.getPosition().getY()) <= 2
					...
					nexNpc.performAnimation(new Animation(6326));
					nexNpc.performGraphic(new Graphic(378));
					new CombatHitTask(nexNpc.getCombatBuilder(), new CombatContainer(NEX, p, 1, CombatType.MAGIC, true))
							.handleAttack();
					return true;
				}
		...
		if (phase == 4) {
			...
			if (prayerTimer == 4) {
				if (prayerType == 0) {
					prayerType = 1;
					nexNpc.setTransformationId(13448);
					nexNpc.getUpdateFlag().flag(Flag.TRANSFORM);
				} else {
					prayerType = 0;
					nexNpc.setTransformationId(13450);
					nexNpc.getUpdateFlag().flag(Flag.TRANSFORM);
				}
				prayerTimer = 0;
			}
			if (p.getPosition().distanceToPoint(nexNpc.getPosition().getX(), nexNpc.getPosition().getY()) <= 2
				...
				nexNpc.performAnimation(new Animation(6326));
				nexNpc.performGraphic(new Graphic(373));
				new CombatHitTask(nexNpc.getCombatBuilder(), new CombatContainer(NEX, p, 1, CombatType.MAGIC, true))
						.handleAttack();
				return true;
			}
		...
		if (phase == 3) {
			...
			if (rnd >= 0 && rnd <= 3 && !attacks[0]) {
				attacks[0] = true;
				nexNpc.forceChat("Die now, in a prison of ice!");
				nexNpc.setChargingAttack(true);
				...
				nexNpc.forceChat("Contain this!");
				nexNpc.setChargingAttack(true);
				attacks[1] = true;
				final int origX = nexNpc.getPosition().getX(), origY = nexNpc.getPosition().getY();
	...
	public boolean checkAttack(Player p, int npc) {
		if (npc == 13447) {
	...
	public void cough(final Player p) {
		if (p.isCoughing())
			return;

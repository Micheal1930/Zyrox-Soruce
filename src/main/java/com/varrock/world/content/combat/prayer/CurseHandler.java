package com.varrock.world.content.combat.prayer;
...
import com.varrock.world.content.ItemsKeptOnDeath;
...
public class CurseHandler {
	...
	public static void handleLeech(GameCharacter p, int index, int amount, int maxAmount, boolean decrease) {
		if(!p.isPlayer())
			return;
	...
	public static void activateCurse(Player player, CurseData curse) {
		...
		if(System.currentTimeMillis() - player.getDragonScimInjury() <= 4000) {
			player.getPacketSender().sendMessage("You must recover from your injury first...");
		...
		if (player.prayblock == 1) {
			if (curse == CurseData.DEFLECT_MAGIC || curse == CurseData.DEFLECT_MELEE ||
				...
				player.getPacketSender().sendConfig(curse.configId, 0);
				player.getPacketSender().sendMessage("timer: "+player.getProtPrayDelay().getTime()+"");
				return;
			}
		}
		player.getProtPrayDelay().reset();
	...
	private static void startDrain(final Player player) {
		...
		TaskManager.submit(new Task(1, player, true) {
			@Override
			public void execute() {
				//if((player.getRights() == PlayerRights.EXTREME_DONATOR || player.getRights() == PlayerRights.PLATINUM_DONATOR || player.getRights() == PlayerRights.EXECUTIVE_DONATOR) && player.getLocation() != Location.WILDERNESS)
				//	return;
				if((player.getRights() == PlayerRights.MODERATOR || player.getRights() == PlayerRights.ADMINISTRATOR || player.getRights() == PlayerRights.DEVELOPER) && player.getLocation() != Location.WILDERNESS) {
					return;
				}
				if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
					for (CurseData curse : CurseData.values()) {
			...
			/*	if(player.getLocation() != Location.WILDERNESS) {
					if(player.getRights() == PlayerRights.EXTREME_DONATOR || player.getRights() == PlayerRights.PLATINUM_DONATOR || player.getRights() == PlayerRights.EXECUTIVE_DONATOR) {
						drain = 0;
					}
				} */
				if (drain <= 0) {

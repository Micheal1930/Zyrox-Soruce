...
public class DwarfMultiCannon {
	...
	CANNON_BASE = 100_007, CANNON_STAND = 100_008,
	CANNON_BARRELS = 100_009, CANNON = 100_006,
	...
	public static void setupCannon(final Player c) {
		if(c.getLocation() == Location.IN_JAIL || c.getLocation() == Location.GAMBLING || c.getLocation() == Location.VARROCK || c.getLocation() == Location.EDGE || c.getLocation() == Location.DUEL_ARENA ||c.getLocation() == Location.DROPPARTY ) {
			c.getPA().sendMessage("You can't do that here");
			return;
		}
		if(!canSetupCannon(c))
			return;
	...
	public static boolean canSetupCannon(Player c) {
		if(c.getSkillManager().getCurrentLevel(Skill.RANGED) < 40) {
			c.getPacketSender().sendMessage("You need a Ranged level of atleast 40 to setup the dwarf-cannon.");
			return false;
		}
	...
	public static void pickupCannon(Player c, DwarfCannon cannon, boolean force) {
		...
		if(force) { //Logout
			if(c.getInventory().getFreeSlots() >= 5) {
				c.getInventory().add(CANNON_BASE_ID, 1, "Pickup cannon");
				c.getInventory().add(CANNON_STAND_ID, 1, "Pickup cannon");
				c.getInventory().add(CANNON_BARRELS_ID, 1, "Pickup cannon");
				c.getInventory().add(CANNON_FURNACE_ID, 1, "Pickup cannon");
				if(addCannonballs)
					c.getInventory().add(CANNONBALL, cannon.getCannonballs(), "Pickup cannon");
				deleted = true;
			} else {
				c.getBank(c.getCurrentBankTab()).add(CANNON_BASE_ID, 1, "Pickup cannon");
				c.getBank(c.getCurrentBankTab()).add(CANNON_STAND_ID, 1, "Pickup cannon");
				c.getBank(c.getCurrentBankTab()).add(CANNON_BARRELS_ID, 1, "Pickup cannon");
				c.getBank(c.getCurrentBankTab()).add(CANNON_FURNACE_ID, 1, "Pickup cannon");
				if(addCannonballs)
					c.getBank(c.getCurrentBankTab()).add(CANNONBALL, cannon.getCannonballs(), "Pickup cannon");
				deleted = true;
			}
		} else {
			if(c.getInventory().getFreeSlots() >= 5) {
	...
	public static void attack(Player player) { //TODO: Bad hardcoding of setting damage, switch to combathittask 
		...
		int combatXp = ((((dmg.getDamage() * 2) * Skill.RANGED.getModifier()) / 10));
		player.getSkillManager().addExperience(Skill.RANGED, combatXp);

package com.varrock.world.entity.updating;

import java.util.Iterator;

import com.varrock.model.Appearance;
...
import com.varrock.model.UpdateFlag;
...
import com.varrock.net.packet.ValueType;
import com.varrock.net.packet.Packet.PacketType;
import com.varrock.net.packet.PacketBuilder.AccessType;
...
public class PlayerUpdating {
	...
	public static void update(final Player player) {
		...
		for (Iterator<Player> playerIterator = player.getLocalPlayers().iterator(); playerIterator.hasNext();) {
			...
			if (World.getPlayers().get(otherPlayer.getIndex()) != null && otherPlayer.isVisible() && !otherPlayer.isHidePlayer() && !otherPlayer.isSpectatorMode() && otherPlayer.getPosition().isWithinDistance(player.getPosition()) && !otherPlayer.isNeedsPlacement()) {
		...
		for(Player otherPlayer : World.getPlayers()) {
			if (player.getLocalPlayers().size() > 255 || playersAdded > MAX_NEW_PLAYERS_PER_CYCLE)
				break;
			if (otherPlayer == null || otherPlayer == player || !player.isVisible() || otherPlayer.isHidePlayer() || otherPlayer.isSpectatorMode() || player.getLocalPlayers().contains(otherPlayer) || !otherPlayer.getPosition().isWithinDistance(player.getPosition()))
				continue;
	...
	private static void updateChat(PacketBuilder builder, Player target) {
		...
		builder.put(target.getGameMode().getId());
		builder.put(target.prestige);
	...
	private static void updateAppearance(Player player, PacketBuilder out, Player target) {	
		...
		properties.put(appearance.getLook()[Appearance.GENDER]);
		...
		if (target.getNpcTransformationId() <= 0) {
			...
			if (equip[Equipment.CAPE_SLOT] > -1) {
				...
				if(equip[Equipment.CAPE_SLOT] == 14019 || equip[Equipment.CAPE_SLOT] == 14022) {
					int[] modelColors = equip[Equipment.CAPE_SLOT] == 14022 ? target.getCompCapeColors() : target.getMaxCapeColors();
					if(modelColors != null) {
			...
			if (appearance.getLook()[Appearance.BEARD] <= 0 || appearance.getGender().equals(Gender.FEMALE) || equip[Equipment.HEAD_SLOT] == 1588 || equip[Equipment.HEAD_SLOT] == 1589 || equip[Equipment.HEAD_SLOT] == 1590 || equip[Equipment.HEAD_SLOT] == 1591) {
				properties.put(0);
			} else {
		...
		if(skillAnim > 0) {
			...
			properties.putShort(target.getCharacterAnimations().getStandTurnAnimation());
			properties.putShort(target.getCharacterAnimations().getWalkingAnimation());
			properties.putShort(target.getCharacterAnimations().getTurn180Animation());
			properties.putShort(target.getCharacterAnimations().getTurn90CWAnimation());
			properties.putShort(target.getCharacterAnimations().getTurn90CCWAnimation());
			properties.putShort(target.getCharacterAnimations().getRunningAnimation());
		}

		properties.putLong(target.getLongUsername());
		properties.put(target.prestige);
		properties.put(target.getSkillManager().getCombatLevel());
		properties.putShort(target.getRights().getId());
		properties.putString(target.getTitle() == null ? "" : target.getTitle());

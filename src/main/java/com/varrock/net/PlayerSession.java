...
import com.varrock.net.packet.PacketConstants;
import com.varrock.net.packet.PacketListener;
import com.varrock.net.packet.codec.PacketDecoder;
import com.varrock.net.packet.impl.PlayerRelationPacketListener;
...
public final class PlayerSession {
	...
	public void handleInputMessage(Packet msg) {
		...
		try {
			...
			if(op != 11 && op != 60 && op != 5 && op != 12 && op != 103 && op != 182 && op != 230 && op != 4 && op != 98 && op != 164 && op != 248 && op != 123 && !(listener instanceof PlayerRelationPacketListener)) {
			...
			if(cycle >= 500) {
				System.err.println(cycle+"ms - "+op+" - "+listener.getClass().getSimpleName());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

package com.varrock.net.packet.codec;
...
import com.varrock.net.security.IsaacRandom;
...
public final class PacketEncoder extends OneToOneEncoder {
	...
	protected Object encode(ChannelHandlerContext context, Channel channel,
		...
		if (packetType == PacketType.BYTE) {
			headerLength += 1;
			if (packetLength >= 256) {
				System.out.println("The packet length sent is too long for a byte sized packet: "+packet.getOpcode()+" - "+packet.getSize());
				return packet.getBuffer();
			...
			if (packetLength >= 65536) {
				System.out.println("The packet length sent is too long for a short sized packet: "+packet.getOpcode()+" - "+packet.getSize());
				return packet.getBuffer();

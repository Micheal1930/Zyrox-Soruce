...
import com.varrock.model.log.impl.PrivateMessageLog;
...
public class PlayerRelations {
	...
	public void message(Player friend, byte[] message, int size) {
        ...
        if (player.getRelations().ignoreList.contains(friend.getLongUsername()) || friend.getRelations().ignoreList.contains(player.getLongUsername())) {
            return;
        }
		...
		if (Misc.blockedWord(msg)) {
			DialogueManager.sendStatement(player, "Your text contains forbidden words.");
			return;
		}

		new PrivateMessageLog(player.getName(), friend.getName(), msg, Misc.getTime()).submit();

		friend.getPacketSender().sendPrivateMessage(player.getLongUsername(), player.getGameMode(), player.getRights(), player.prestige, message, size);
	}
